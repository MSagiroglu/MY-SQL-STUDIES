...
insert into doktorlar values('Hasan','Esma',123456,'noroloji');

--SORU 2: .doktorlar tablosuna gorev_yeri 'Okmeydanı' olan veri girişi yapınız.
insert into doktorlar values('Ok meydanı','Hatice Kar','456789','ortopedi');
-->Parent tabloda olmayan primary key ile child tabloya giriş yapılamaz
...
-->child tablo null değer kabul eder
select * from doktorlar;
...
delete from hastaneler where hastane_adi='Meddem';
-->Child daki veri silindiği için artık silinebilir.
...
drop table hastaneler;
-->child tablo silindiğinden dolayı parent tablo silinebilir.
...
select min(fiyat) as endusukk_fiyat from arac;

--SORU3 : arac tablosundaki fiyatların toplamını listele
select sum(fiyat) as tumfiyat_toplami from arac;

--SORU4 : arac tablosundaki fiyat ortalamalarını listele
select avg(fiyat)as fiyat_ortalamasi from arac;
select round(avg(fiyat)) as fiyat_ortalamasi from arac;
-->round ondalıklı sayıdan kurtulmak için veya kaç basamak istediğimizi belitmeye yarar.
select round(avg(fiyat), 2) as fiyat_ortalamasi from arac;
